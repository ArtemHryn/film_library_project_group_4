 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */var He;He="",Ne(new J("platform-logger",(e=>new Te(e)),"PRIVATE")),Ne(new J("heartbeat",(e=>new qe(e)),"PRIVATE")),De("@firebase/app","0.7.33",He),De("@firebase/app","0.7.33","esm2017"),De("fire-js","");
/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function $o(e){return 1e3*Number(e)}function Ho(e){var t;const[n,r,i]=e.split(".");if(void 0===n||void 0===r||void 0===i)return go("JWT malformed, contained fewer than 3 sections"),null;try{const e=g(r);return e?JSON.parse(e):(go("Failed to decode base64 JWT payload"),null)}catch(e){return go("Caught error parsing JWT payload as JSON",null===(t=e)||void 0===t?void 0:t.toString()),null}}
/**
 * @license
 * Copyright 2020 Google LLC
 *
 ...
 * limitations under the License.
 */
class Yo{get isExpired(){return!this.expirationTime||Date.now()>this.expirationTime-3e4}updateFromServerResponse(e){bo(e.idToken,"internal-error"),bo(void 0!==e.idToken,"internal-error"),bo(void 0!==e.refreshToken,"internal-error");const t="expiresIn"in e&&void 0!==e.expiresIn?Number(e.expiresIn):function(e){const t=Ho(e);return bo(t,"internal-error"),bo(void 0!==t.exp,"internal-error"),bo(void 0!==t.iat,"internal-error"),Number(t.exp)-Number(t.iat)}(e.idToken);this.updateTokensAndExpiration(e.idToken,e.refreshToken,t)}async getToken(e,t=!1){return bo(!this.accessToken||this.refreshToken,e,"user-token-expired"),t||!this.accessToken||this.isExpired?this.refreshToken?(await this.refresh(e,this.refreshToken),this.accessToken):null:this.accessToken}clearRefreshToken(){this.refreshToken=null}async refresh(e,t){const{accessToken:n,refreshToken:r,expiresIn:i}=await
/**
 * @license
 * Copyright 2020 Google LLC
 *
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class $a extends qa{_addListener(e,t){}_removeListener(e,t){}constructor(){super((()=>window.sessionStorage),"SESSION")}}$a.type="SESSION";const Ha=$a;
/**
 * @license
 * Copyright 2019 Google LLC
 *
 ...
 * limitations under the License.
 */
function(e=xe()){const t=Oe(e,"auth");return t.isInitialized()?t.getImmediate():function(e,t){const n=Oe(e,"auth");if(n.isInitialized()){const e=n.getImmediate();if(U(n.getOptions(),null!=t?t:{}))return e;mo(e,"already-initialized")}return n.initialize({options:t})}(e,{popupRedirectResolver:jc,persistence:[nc,Wa,Ha]})}();var Yc,Jc,Qc;Yc=e=>{e&&(zc.USER_ID=e.uid,zc.userName=e.displayName,zc.IsLogIn=!0,zc.userImg=e.photoURL,function(e,t){const n=`<img\n        class="user-img"\n        src='${e}'\n        alt="photo"\n      /><ul class='popup'>\n      <li class='popup-item'><span class='user-name'>${t}</span></li>\n      <li class='popup-item'><button type='button' class='logout-btn' data-value='logout'>Log Out</button></li>\n      </ul>`;ze.login.innerHTML=n}(zc.userImg,zc.userName),(ze.logout=document.querySelector('[data-value="logout"]')).addEventListener("click",Hc),Ye(ze.login,"login-text"))},Y(Kc).onAuthStateChanged(Yc,Jc,Qc);const Xc=co();function Zc(e){try{to(Zs(Xc,`films/${zc.UserID}/${e.id}`),e)}catch(e){console.log(e)}}const el=co();function tl(e){const t=e?`films/${zc.UserID}/${e}`:`films/${zc.UserID}/`;return no(Zs(el,t)).then((t=>{if(t.exists())return e?t.val():Object.values(t.val());console.log("No data available")})).catch((e=>{console.error(e)}))}function nl(){ze.spinner.classList.contains("loader")||(Ge(ze.spinner,"loader"),Ke(ze.spinner,"visually-hidden"))}function rl(){ze.spinner.classList.contains("loader")&&(Ge(ze.spinner,"visually-hidden"),Ke(ze.spinner,"loader"))}function il(e){const t=sl("films"),n=t?JSON.parse(t):[];if(!n.find((t=>t.id===e.id)))return n.push(e),void ol(n);const r=function(e,t){return e.map((e=>e.id===t.id?t:e))}(n,e);ol(r)}function sl(e="films"){return localStorage.getItem(e)}function ol(e){localStorage.setItem("films",JSON.stringify(e))}function al(e){const t=sl();if(!t)return;return JSON.parse(t).find((t=>t.id===e))}function cl(e,t){if(!t)return;const n=sl("films");if(!n)return;ol(JSON.parse(n).filter((t=>t.id!==e.id)))}const ll=co();function hl(e,t){if(t)try{return n=Zs(ll,`films/${zc.UserID}/${e}`),_s("remove",n._path),to(n,null)}catch(e){console.log(e)}var n}let ul,dl=!1;function fl(e){e.target.playVideo()}let pl=!1;function gl(e){e.data!=YT.PlayerState.PLAYING||pl||(pl=!0)}function ml(){dl&&(ul.destroy(),dl=!1)}function _l(){ze.trailerModal.removeEventListener("click",vl)}function yl(e){if(console.log(window.screen.width),window.screen.width<1280){const t=e.target.dataset.trailerid;return void parent.open(`https://www.youtube.com/watch?v=${t}`)}Ye(ze.trailerModal,"is-hidden"),Ye(ze.body,"no-scroll"),ze.trailerModal.addEventListener("click",vl);const t=e.target.closest("[data-trailerId]");var n;n=t.dataset.trailerid,console.log(window.location.href),ul=new YT.Player("player",{height:"720",width:"1280",videoId:`${n}`,enablejsapi:1,origin:window.location.href,host:"https://www.youtube.com",playerVars:{playsinline:1},events:{onReady:fl,onStateChange:gl}}),dl=!0,removeListenerForTrailer()}function vl(e){e.currentTarget===e.target&&(Ye(ze.trailerModal,"is-hidden"),Ye(ze.body,"no-scroll"),ml(),_l(),addListenerFromTrailer())}function wl(e){"Escape"==e.code&&(Ye(ze.trailerModal,"is-hidden"),ml(),document.removeEventListener("keydown",wl),_l(),addListenerFromTrailer())}const bl=new class{async fetchMovies(){try{return(await n(Ve).get(`https://api.themoviedb.org/3/trending/movie/week?api_key=29563162ad0b73335f880d56505c78bf&page=${this.page}`)).data}catch(e){throw new Error("Oops, something went wrong")}}async fetchGenres(){try{return(await n(Ve).get("\nhttps://api.themoviedb.org/3/genre/movie/list?api_key=29563162ad0b73335f880d56505c78bf&language=en-US")).data.genres}catch(e){throw new Error("Oops, something went wrong")}}resetPage(){this.page=1}set Page(e){this.page=e}incrementPage(){this.page+=1}decrement(){this.page-=1}get film(){return this.addFilm}set film(e){this.addFilm=e}constructor(){this.page=1,this.genres=this.fetchGenres(),this.addFilm={}}},Il=new class{async fetchMovies(){try{return(await n(Ve).get(`https://api.themoviedb.org/3/movie/${this.movieId}?api_key=29563162ad0b73335f880d56505c78bf`)).data}catch(e){throw new Error("Oops, something went wrong")}}async fetchTreiler(){try{return(await n(Ve).get(`https://api.themoviedb.org/3/movie/${this.movieId}/videos?api_key=29563162ad0b73335f880d56505c78bf&language=en-US`)).data.results}catch(e){throw new Error("Oops, something went wrong")}}get id(){return this.movieId}set id(e){return this.movieId=e}constructor(){this.movieId=null}},El=new class{async fetchMovies(){try{return(await n(Ve).get(`https://api.themoviedb.org/3/search/movie?api_key=29563162ad0b73335f880d56505c78bf&query=${this.searchQuery}&page=${this.page}`)).data}catch(e){throw new Error("Oops, something went wrong")}}resetPage(){this.page=1}set Page(e){this.page=e}incrementPage(){this.page+=1}decrement(){this.page-=1}get query(){return this.searchQuery}set query(e){this.searchQuery=e}get films(){return this.listOfFilms}set films(e){this.listOfFilms=e}constructor(){this.searchQuery="",this.page=1,this.listOfFilms={}}};async function Tl(e){return zc.isLogIn?await tl(e):al(e)}function Cl(){document.querySelector(".js-film-modal").remove(),ze.backdrop.classList.add("is-hidden"),window.removeEventListener("keydown",Pl),ze.backdrop.removeEventListener("click",Rl),document.body.style.overflow="auto",_l()}function kl(e,t,n){const r=function(e){return(""!==El.query?El.films:bl.film.results).filter((t=>t.id===e))}(+e.target.closest("[data-id]").dataset.id);return{...r[0],isWatched:t,isQueue:n}}async function Sl(e){try{nl();const t=await async function(){return zc.isLogIn?await tl():JSON.parse(sl())}();if(!t)return void(ze.filmContainer.innerHTML="");const n=t.filter((t=>t[e]));if(0===n.length)return void(ze.filmContainer.innerHTML="");ze.filmContainer.innerHTML=Je(n,await bl.fetchGenres()),setInterval((()=>rl()),2e3),Qe()}catch(e){console.log(e)}}function Pl(e){"Escape"===e.code&&Cl()}function Rl(e){e.target===ze.backdrop&&Cl()}async function Nl(){Sl("isWatched")}async function Ol(){Sl("isQueue")}async function Al(e){const t=kl(e,!0,!1),n=await Tl(t.id);return zc.isLogIn&&n&&n.isWatched?(hl(n.id,n.isWatched),void Cl()):!zc.isLogIn&&n&&n.isWatched?(cl(n,n.isWatched),void Cl()):(zc.isLogIn?Zc(t):il(t),void Cl())}async function Ll(e){const t=kl(e,!1,!0),n=await Tl(t.id);return zc.isLogIn&&n&&n.isQueue?(hl(n.id,n.isQueue),void Cl()):!zc.isLogIn&&n&&n.isQueue?(cl(n,n.isQueue),void Cl()):(zc.isLogIn?Zc(t):il(t),void Cl())}function xl(){ze.wached=document.querySelector('[data-value="wached"]'),(ze.queue=document.querySelector('[data-value="queue"]')).addEventListener("click",Vl),ze.wached.addEventListener("click",zl)}function Dl(){ze.form=document.querySelector("#search-form"),ze.erorr=document.querySelector(".search-error"),ze.form.addEventListener("submit",Gl)}function Ml(){ze.addToWatched=document.querySelector(".js-add-to-watched"),ze.addtoQueue=document.querySelector(".js-add-to-queue"),ze.closeModalFilm=document.querySelector("[data-modal-close]"),ze.addToWatched.addEventListener("click",Al),ze.addtoQueue.addEventListener("click",Ll),ze.closeModalFilm.addEventListener("click",Cl)}function Ul(){ze.container.innerHTML=""}ze.filmContainer.addEventListener("click",(async function(e){if(e.preventDefault(),"IMG"!==e.target.nodeName)return;const t=+e.target.closest("[data-id]").dataset.id;Il.movieId=t;const n=await async function(){try{const e=await Il.fetchTreiler();return e.find((e=>"Official Trailer"===e.name||"Official Teaser"===e.name?e:void 0))}catch(e){console.log(e)}}(),r=n?n.key:void 0,i={...await Il.fetchMovies(),key:r,isTrailer:Boolean(r)};await bl.fetchGenres();ze.backdrop.classList.remove("is-hidden");const s=await Tl(t);if(s){const{isQueue:e,isWatched:t}=s;ze.backdrop.innerHTML=Ze({...i,isWatched:t,isQueue:e})}else ze.backdrop.innerHTML=Ze(i);Ml(),(ze.dataOpenTrailer=document.querySelector("[data-open-trailer]")).addEventListener("click",yl),ze.trailerModal.addEventListener("click",vl),document.addEventListener("keydown",wl),window.addEventListener("keydown",Pl),ze.backdrop.addEventListener("click",Rl),document.body.style.overflow="hidden"})),ze.logo.addEventListener("click",Hl),ze.home.addEventListener("click",Hl),ze.library.addEventListener("click",(function(e){Ul(),ze.container.innerHTML='<div class=\'btn-flex\'>\n  <button type="button" class="lib-btn" data-value="wached">Watched</button>\n    <button type="button" class="lib-btn" data-value="queue">queue</button>\n    </div>',Ge(ze.library,"current-page"),Ke(ze.home,"current-page"),Ke(ze.header,"header-home"),Ge(ze.header,"header-library"),xl(),e.target.blur(),Vl(),ze.form.removeEventListener("submit",Gl),Ol()})),Dl();function Fl(e,t,n){var r,i=n||{},s=i.noTrailing,o=void 0!==s&&s,a=i.noLeading,c=void 0!==a&&a,l=i.debounceMode,h=void 0===l?void 0:l,u=!1,d=0;function f(){r&&clearTimeout(r)}function p(){for(var n=arguments.length,i=new Array(n),s=0;s<n;s++)i[s]=arguments[s];var a=this,l=Date.now()-d;function p(){d=Date.now(),t.apply(a,i)}function g(){r=void 0}u||(c||!h||r||p(),f(),void 0===h&&l>e?c?(d=Date.now(),o||(r=setTimeout(h?g:p,e))):p():!0!==o&&(r=setTimeout(h?g:p,void 0===h?e-l:e)))}return p.cancel=function(e){var t=(e||{}).upcomingOnly,n=void 0!==t&&t;f(),u=!n},p}function Bl(){let e="",t="",n="",r="";if(Wl.currentPage>=6&&(n=`<li data-page="${Wl.FirstPage}" \n    class="pagination-item">\n    <button type="button" class="pagination-btn ">${Wl.FirstPage}\n    </button>\n    </li>\n    <li \n    class="pagination-item ">\n    <button type="button" class="pagination-btn pagi">...\n    </button>\n    </li>`),Wl.currentPage<=Wl.totalPages-5&&(r=`<li \n    class="pagination-item ">\n    <button type="button" class="pagination-btn pagin">...\n    </button>\n    </li>\n    <li data-page="${Wl.totalPages}" \n    class="pagination-item">\n    <button type="button" class="pagination-btn">${Wl.totalPages}\n    </button>\n    </li>\n    `),Wl.totalPages<=5)for(let t=Wl.currentPage>=Wl.totalPages-3?Wl.currentPage>=Wl.totalPages-2?Wl.totalPages<=4?Wl.firstPage:Wl.currentPage-(4-(Wl.totalPages-Wl.currentPage)):Wl.firstPage:Wl.currentPage<=3?Wl.currentPage-(Wl.currentPage-1):Wl.currentPage-2;t<=Wl.totalPages;t+=1){e+=`<li data-page="${t}" class="pagination-item ${t===Wl.currentPage?"active":""}">\n    <button type="button" class="pagination-btn">${t}</button></li>`}else for(let t=Wl.currentPage>=Wl.totalPages-3?Wl.currentPage>=Wl.totalPages-2?Wl.currentPage-(4-(Wl.totalPages-Wl.currentPage)):Wl.currentPage-2:Wl.currentPage<=3?Wl.currentPage-(Wl.currentPage-1):Wl.currentPage-2;t<=(Wl.currentPage>=Wl.totalPages-3?Wl.currentPage>=Wl.totalPages-2?Wl.currentPage+(Wl.totalPages-Wl.currentPage):Wl.currentPage+2:Wl.currentPage<=3?Wl.currentPage+(5-Wl.currentPage):Wl.currentPage+2);t+=1){e+=`<li data-page="${t}" class="pagination-item ${t===Wl.currentPage?"active":""}">\n    <button type="button" class="pagination-btn">${t}</button></li>`}t=window.screen.width<481?e:n+e+r,ze.paginationContainerRef.innerHTML=t}const ql=document.querySelector(".turn-to-top__btn");function jl(){window.scrollTo({top:0,behavior:"smooth"})}document.addEventListener("scroll",(()=>{window.scrollY>634?ql.classList.toggle("visually-hidden",!1):ql.classList.toggle("visually-hidden",!0)})),ql.addEventListener("click",(()=>{jl()}));const Wl=new class{get currentPage(){return this.CurrentPage}set currentPage(e){this.CurrentPage=e}get totalPages(){return this.TotalPages}set totalPages(e){this.TotalPages=e}get firstPage(){return this.FirstPage}set firstPage(e){this.FirstPage=e}resetPage(){this.CurrentPage=1}constructor(){this.CurrentPage=1,this.TotalPages=50,this.FirstPage=1}},$l=new class{changetrendingApi(){this.page?this.trending():this.saerch()}async saerch(){nl(),El.Page=Wl.currentPage;const e=await El.fetchMovies();Wl.TotalPages=e.total_pages,console.log("~ lookedForFilms",e),Bl(),El.films=e.results;const t=await bl.genres;ze.filmContainer.innerHTML=Je(El.films,t),Qe(),setTimeout((()=>{rl()}),1e3)}async trending(){nl(),bl.Page=Wl.currentPage;const e=await bl.fetchMovies();Wl.TotalPages=e.total_pages,Bl(),bl.film=e;const t=await bl.fetchGenres();ze.filmContainer.innerHTML=Je(e.results,t),Qe(),setTimeout((()=>{rl()}),1e3)}set Page(e){this.page=e}get Page(){return this.page}resetALLpage(){Wl.resetPage()}constructor(){this.page=!0}};function Hl(e){ze.queue&&(ze.queue.removeEventListener("click",Vl),ze.wached.removeEventListener("click",zl)),Ul(),ze.container.innerHTML='<form class="search-form" id="search-form">\n      <input\n        type="text"\n        name="searchQuery"\n        autocomplete="off"\n        placeholder="Movie search"\n        class="input-search"\n      />\n      <button type="submit" class="submit-btn">\n        <svg width="15" height="15" class=\'search-svg\'>\n          <use href="/sprite.f14d31f7.svg#icon-search"></use>\n        </svg>\n      </button>\n    </form>\n    <p class="search-error visually-hidden">\n      Search result not successful. Enter the correct movie name and\n    </p>',Ge(ze.home,"current-page"),Ke(ze.library,"current-page"),Ke(ze.header,"header-library"),Ge(ze.header,"header-home"),Dl(),$l.resetALLpage(),$l.Page=!0,$l.changetrendingApi(),e.target.blur()}function Vl(e){Ge(ze.queue,"lib-btn-current"),Ke(ze.wached,"lib-btn-current"),Ol(),e.target.blur()}function zl(e){Ge(ze.wached,"lib-btn-current"),Ke(ze.queue,"lib-btn-current"),Nl(),e.target.blur()}function Gl(e){e.preventDefault(),e.currentTarget.elements.searchQuery.value.trim()?(Ge(ze.erorr,"visually-hidden"),El.searchQuery=e.currentTarget.elements.searchQuery.value.trim(),$l.resetALLpage(),$l.Page=!1,$l.changetrendingApi()):Ke(ze.erorr,"visually-hidden")}$l.changetrendingApi(),ze.paginationContainerRef.addEventListener("click",Fl(700,(function(e){if("BUTTON"!==e.target.nodeName)return;if(!e.target.closest("[data-page]"))return;const t=e.target.closest("[data-page]");Wl.currentPage=+t.dataset.page,Bl(),$l.changetrendingApi(),jl()}))),ze.prevBtnRef.addEventListener("click",Fl(700,(function(){if(Wl.currentPage<=1)return;Wl.currentPage-=1,Bl(),$l.changetrendingApi(),jl()}))),ze.nextBtnRef.addEventListener("click",Fl(700,(function(){if(Wl.currentPage===Wl.totalPages)return;Wl.currentPage+=1,Bl(),$l.changetrendingApi(),jl()})));const Kl=document.createElement("script");Kl.src="https://www.youtube.com/iframe_api";const Yl=document.getElementsByTagName("script")[0];Yl.parentNode.insertBefore(Kl,Yl);const Jl=document.querySelector(".footer__modal-open"),Ql=document.querySelector(".backdrop-team"),Xl=document.querySelector(".modal-team__close-btn");function Zl(){Ql.classList.toggle("backdrop-team--is-hidden")}function eh(){Zl(),Xl.removeEventListener("click",eh),document.body.removeAttribute("style")}function th(e){e.currentTarget===e.target&&(document.body.removeAttribute("style"),Zl(),document.removeEventListener("keydown",nh),Ql.removeEventListener("click",th))}function nh(e){"Escape"==e.code&&(Zl(),document.removeEventListener("keydown",nh),Xl.removeEventListener("click",eh),Ql.removeEventListener("click",th))}Jl.addEventListener("click",(function(e){e.preventDefault(),Zl(),document.addEventListener("keydown",nh),document.body.style.overflow="hidden",Xl.addEventListener("click",eh),Ql.addEventListener("click",th)})),window.setTimeout((function(){document.body.classList.add("loadedhiding"),document.body.classList.add("loaded"),document.body.classList.remove("loadedhiding")}),1500);const rh=document.querySelector("body");document.querySelector(".switch").addEventListener("change",(function(){rh.classList.contains("dark-theme")?(Ke(rh,"dark-theme"),Ge(rh,"light-theme")):(Ke(rh,"light-theme"),Ge(rh,"dark-theme"))}));
//# sourceMappingURL=index.73748701.js.map
